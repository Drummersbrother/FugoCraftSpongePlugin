...
public class PlayerHelpers {
	 ...
	 * 
	 * @param Inventory
	 *            inventory. This inventory must be compatible with all slots in
	 *            a Player, I think this means that it has to be a
	 *            HumanInventory
	 * @param Player
	 *            target.
	...
	public static Player setPlayerInventory(Inventory inventory, Player target) {
		...
		// Checking if the passed inventory is a HumanInventory (or something, I don't really understand it all)
		if (inventory.query(HumanInventory.class).isEmpty() != true) {
			
			// Clear the inventory to prepare for a new inventory
			target.getInventory().clear();
		
			// This should work, if it throws a NPE it is not my fault (I think) as I have already checked that
